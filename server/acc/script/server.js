...
if (!process.env.JWT_SECRET) {
  if (config.nodeEnv === 'production') {
    ...
    console.warn('⚠  Missing JWT_SECRET - using default for development');
...
const loadRoutes = () => {
  ...
  try {
    ...
    if (fs.existsSync(authRoutesPath)) {
      ...
      console.log('⚠  Auth routes not found, using fallback');
    }
  } catch (err) {
  ...
  try {
    ...
    if (fs.existsSync(chatRoutesPath)) {
      ...
      console.log('⚠  Chat routes not found, using fallback');
    }
  } catch (err) {
